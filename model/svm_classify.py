...
def svm_class():
    ...
    # 总标签list
    label_list = list()
    with open(conf["label_path"], "r", encoding="utf-8") as f:
        lines = f.readlines()
        for line in lines:
            label = line.strip()
            label_list.append(label)

    model = FastText.load(conf["model_path"])
    with open(conf["shorten_corpus_path"], "r", encoding="utf-8") as file:
        ...
        for line in lines:
            ...
            #     temp.append(item)
            #--------------------------------------------------
            pos_vector = get_feature.get_pos_feature(line.strip())
